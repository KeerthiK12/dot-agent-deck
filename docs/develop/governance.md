...
**In place is not the same as exercised, and this specific combination has not been.** The two mechanisms are separately verified. The bypass actors survived the full `PUT` that added the checks — `gh api repos/vfarcic/dot-agent-deck/rulesets/20587589` still reports `actor_id: 5` (`RepositoryRole`, `bypass_mode: always`) and `actor_id: 2740` (`Integration`, `bypass_mode: pull_request`), which `scripts/apply-branch-protection.sh status` prints alongside the required contexts. The token has pushed to a `main` protected by the `pull_request` rule, most recently for v0.35.10. What has *not* happened is a direct push to `main` since `required_status_checks` returned at 2026-08-11T15:00:51Z: the last release, v0.35.10, was published the day before at 2026-08-10T17:21Z. Since the v0.35.6 failure was specifically a required-status-checks rejection on a check-run-less direct push, that is the one leg still untested — so **run `scripts/apply-branch-protection.sh status` before the next tag**, confirm the admin bypass is still listed, and prefer [step 5's canary](#rollout) over a real release as the first thing to point at it. `bypass_actors` is admin-only, so a `write` maintainer cannot check this and should ask rather than assume.
...
The same applies to `scripts/apply-branch-protection.sh` if a fork ever runs it: `REQUIRED_CHECKS` names this repository's four `ci.yml` job ids, and requiring a context the fork's CI never produces leaves every pull request unmergeable with nothing red to fix. Run it with `REQUIRED_CHECKS=` there, or with the fork's own job names.
