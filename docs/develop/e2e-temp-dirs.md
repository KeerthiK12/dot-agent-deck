...
1. **Resolved once.** The longest prefix that already exists is `canonicalize`d and the missing remainder joined onto it, so what the harness works with — and what every later message names, and what the socket-length budget is measured against — is the path the kernel will really walk. Symlinked ancestors are resolved rather than refused. They have to be: on macOS `/var` is a symlink to `/private/var`, so `/var/tmp` and the platform's own `std::env::temp_dir()` have a symlinked ancestor on a completely healthy machine, and refusing them rejected the entire platform. A root-owned system symlink is not the threat model; an attacker-planted component is, and that is judged below, *after* resolution.
...
4. **The base itself is revalidated at the end**, on the descriptor the walk finished on, under the strict rule: a real directory, owned by the effective UID, with **no group or other bits at all**. That is the same bar the default `/var/tmp/dad-e2e-<uid>` parent has to meet, and for the same reason — this is where the harness seeds real agent credentials. It applies whether the harness created the directory or found it, so pointing the variable at an existing 0755 directory now fails with `mode is 0o755, not owner-only`; `chmod 700` is the fix, and the directory is refused rather than repaired.
