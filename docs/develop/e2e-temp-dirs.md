...
| Where | Contained? | How |
|---|---|---|
| `src/dispatch.rs` unit tests | yes, weakly | `crate::test_temp::tempdir()` — the same private base, none of the machinery |
| `tests/daemon_protocol.rs` | yes, weakly | `#[path]`-includes the same `test_temp` module |
| The rest of `src/`'s unit tests, and `tests/rehydration.rs`, `tests/pane_close.rs`, `tests/codex_hooks_safety.rs`, `tests/features.rs`, `tests/devin_hook_ingestion.rs`, `tests/codex_hook_ingestion.rs` | **no** | bare `tempfile::tempdir()`, OS temp dir |
...
Those two files were singled out because measurement singled them out. `src/dispatch.rs`'s e2e-gated dispatch test was observed holding a live 184 KiB `/tmp/.tmpYN3lNF` containing a cloned repo and its worktree — small today, but a repo clone is not structurally bounded. `tests/daemon_protocol.rs` is the one file in the fast-tier group that **binds Unix domain sockets**, and `cargo test-e2e` runs the fast tier too, so it was allocating in `/tmp` during the e2e tier; measured as `/tmp/.tmpVtiW6e/attach.sock`, 0 blocks.

The rest of the table stays uncontained on cost grounds, and the number that justifies it is small: the whole measured leak was **~184 KiB** across 18 transient `/tmp/.tmp*` entries, all gone by the end of the run. These are small L1 allocations with a `TempDir` guard that drops on the normal path, not the repo clones and seeded HOMEs that motivated this issue — those are the 263–284 MB roots, and they are all under `/var/tmp/dad-e2e-<uid>`. Requiring a wrapper across the rest of `src/` would touch ~80 call sites for no measured benefit. After the two fixes above, live sampling of `/tmp` across those tests shows **zero** new entries.
