...
| `Ctrl+L` | Toggle the orchestration tab's sidebar/pane-column split ratio between 34/66 and 25/75 | **Orchestration tabs, command mode only** |
...
| `toggle_orchestration_split` | `Ctrl+l` | Toggle the orchestration tab's sidebar/pane-column split between 34/66 and 25/75 — **orchestration tabs, command mode only**; in a pane, and on every other tab, the chord is ordinary input for whatever is running there |
