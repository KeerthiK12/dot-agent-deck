...
##### orchestration/delegate/016 — The generated orchestrator context names the running binary's own file name, not a baked-in literal (issue #253).
...
- **Does not assert:** the symlink-resolution behavior of `current_exe()` itself (a property of the platform, not this crate); the fallback branch (`orchestration/delegate/018`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/delegate/017 — The generated worker task file's `work-done` instruction names the running binary's own file name, not a baked-in literal (issue #253).
...
- **Does not assert:** the fallback branch (`orchestration/delegate/018`); the rest of the footer's shell-safety content (covered by the pre-existing `compose_worker_task_file_appends_work_done_footer`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/delegate/018 — The command-name resolver falls back to the crate's default literal when `current_exe()` is unavailable or unusable (issue #253).
