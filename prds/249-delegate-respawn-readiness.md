...
**Status**: In progress — M1, M3, M5 and M4's fast tier landed, M4's real-agent e2e written; M2 (pre-fix exposure) and M6 outstanding
**Priority**: High
**Created**: 2026-07-28
...
- [x] **M4**: Slow-readiness toggle test (fails at 0, passes at chosen buffer) plus fast-tier assertions; real-agent e2e written (`tests/e2e_delegate_respawn_readiness.rs`, `/014` Claude and `/015` OpenCode) with `/015` observed green — the full `cargo test-e2e` gate run is the pre-PR step
- [x] **M5**: Docs describe `clear`'s delivery semantics; changelog fragment; rule 12 answer recorded (the rule 12 *manual* previous-release-daemon run is still owed at PR time — the recorded answer is "no `PROTOCOL_VERSION` bump, no `.breaking.md`")
...
### M2 status (as of this round — NOT complete)

- **Post-fix, real agent, PASSES.** `orchestration/delegate/015` drives a real OpenCode worker through a `clear = true` delegate on the normal 1000 ms buffered arm: 1/1 in 45.3 s. Current OpenCode delivery works and no production defect (and no stray silence notice) was reproduced.
- **Pre-fix exposure is NOT yet proven.** The earlier saved OpenCode failure artifact showed the severe lost-payload symptom, but its buffer-zero-vs-default arm cannot be attributed: the parent override was not recorded and the source changed after the cast. Verdict on that artifact: **could-not-observe**, and it was discarded rather than cited.
- **How it gets settled**: a buffer-`0` toggle run of `/015` against the FINAL shipped code, after this round's changes land. The sequencing is deliberate — the discarded artifact is exactly the failure mode of measuring against source that then moves, so the M2 observation must be taken against code that is not going to change under it.

### M5 (docs) — as landed

- **`docs/orchestration.md` → "What `clear` does to delivery"**, a subsection of "How delegation works" (with a pointer from the `clear` row of the `[[orchestrations.roles]]` reference table). This is the user-facing page where `clear` is already documented, per rule 11 — the reader who needs this is someone choosing `clear` for a role, not a contributor reading internals. It covers what the respawn does to the process, why a replacement agent can miss a write, the 1000 ms readiness buffer and its measured 656 ms basis, `DOT_AGENT_DECK_DELEGATE_READINESS_BUFFER_MS` as the operator escape hatch, and `clear = false` as the interim workaround @xclydes and @tomikonio found — framed as "no longer needed on a release with the buffer; set `clear` for the context behaviour you want".
...
- **`docs/idle-workers-and-notifications.md` → "A second report: the worker that never said anything"**, at the end of Part 1. M3's notice is a second daemon report about a worker, so it belongs beside the first one rather than on a new page: the real wording, why it is written unsubmitted (a scrollback line, not a prompt — the opposite choice from PRD #126's report, on purpose), the identity binding, and `DOT_AGENT_DECK_DELEGATE_NO_EVENT_WINDOW_MS` including the point that switching the diagnostic off no longer costs idle-worker detection.
