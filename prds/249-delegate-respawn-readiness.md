...
- **Post-fix, real agent, PASSES.** `orchestration/delegate/015` drives a real OpenCode worker through a `clear = true` delegate on the normal 1000 ms buffered arm: 1/1 in 45.3 s. Current OpenCode delivery works: no production defect was reproduced. It establishes nothing about the M3 silence notice either way — the common e2e harness pins that detector off, so `/015` cannot observe a stray one.
...
**Still owed at PR time** (unchanged by this round): M2's buffer-`0` OpenCode toggle observation against final shipped code, full `cargo test-e2e` with `DOT_AGENT_DECK_RECORD=1`, the rule 12 previous-release-daemon manual run, and M6.
