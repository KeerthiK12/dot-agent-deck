...
**Status**: Complete (2026-07-30) — [PR #271](https://github.com/vfarcic/dot-agent-deck/pull/271) merged as `67302b9` (a merge commit, history preserved), issue #249 closed. M1–M6 all landed: the fix, the visibility report, the docs and the tests, the full `cargo test-e2e` gate green (2,542 passed / 0 failed), M2 settled empirically as a **negative** (OpenCode exposure NOT reproducible in 3 zero-buffer attempts — unproven, not demonstrated), the rule 12 manual previous-release-daemon run passed (no `PROTOCOL_VERSION` bump, no `.breaking.md`, patch bump), and the M6 close-out done: [#199](https://github.com/vfarcic/dot-agent-deck/issues/199) closed with the shipped fix and its limits referenced and all five reporters asked to confirm, and the buffer's retirement dependency recorded against [#243](https://github.com/vfarcic/dot-agent-deck/issues/243), left open as the durable replacement. No `graduate-` follow-up — a correctness fix on an existing delivery path, shipped visible with no `experimental` gate.
**Priority**: High
**Created**: 2026-07-28
...
- [x] **M6**: #199 closed with the fix referenced ([comment](https://github.com/vfarcic/dot-agent-deck/issues/199#issuecomment-5135748633)) and all five reporters asked to confirm; retirement dependency recorded against #243 ([comment](https://github.com/vfarcic/dot-agent-deck/issues/243#issuecomment-5135757556)), which stays open. See "M6 close-out — as posted" below
...
### Greptile PR review (round 6) — production fixes as landed

Greptile's review of PR #271 was advisory (4/5, no `CHANGES_REQUESTED`, "appears safe to merge") with two non-blocking findings. Both were accepted and fixed before merge, because the first is a real functional defect in the feature this PRD ships.

- **`work-done` retires ONE silent-worker watch, oldest-first** (`src/state.rs::handle_work_done`). It used to call the unsequenced `cancel_silence_watch`, and because `arm_silence_watch` inserts by REPLACING, the map holds the *newest* delegation's watch — so a `work-done` belonging to delegation N (late, duplicated or retried) disarmed delegation N+1's watch, and if N+1's pointer never landed no notice was ever emitted: M3's undelivered-prompt detector was silently switched off for exactly the case this PRD exists to surface. The fix mirrors PRD #126 finding 6 rather than borrowing its record: `SilenceWatchRecord` gets its own `superseded` counter, carried forward when a superseding delegate replaces the record, and `retire_silence_watch` decrements it (cancelling nothing) while superseded delegations remain unaccounted for. Deliberately **not** keyed to the idle detector's `OutstandingDelegation.seq`: the two detectors are independently switchable, so with `worker_response_timeout = 0` there is no record to derive a generation from and the timely cancellation would break. It inherits #126's accepted hole in the other direction (an out-of-order completion is credited to the older watch, so the newest may emit one discardable notice for work that is done) — strictly safer than silence, and only reachable in a state the orchestrator protocol already forbids. The unsequenced `cancel_silence_watch` is deleted so the footgun cannot be reintroduced. Pinned by `tests/delegate_prompt_injection.rs::delegate_work_done_cancels_only_matching_silence_watch` in both directions plus `src/agent_pty.rs::retire_silence_watch_applies_work_done_to_the_oldest_watch`.
- **The M1 readiness buffer is cancellable** (`src/state.rs::dispatch_one_owned`). The sleep was unconditional, so a pane closed mid-wait kept the dispatch task alive for the remainder — negligible at the 1000 ms default, up to 30 s at the clamp — before the guarded write discovered the target was gone. The *outcome* was already correct (the B1 guard refuses with `WrongSession`/`Stale`/`NoLiveTarget` and writes nothing), so this is a lingering-task cost, not a correctness risk. New `AgentPtyRegistry::pane_close_signal` hands out a `oneshot` that `begin_pane_close` resolves by dropping the sender — the same drop-to-cancel discipline as both watches' `_cancel` channels — and the gate becomes `tokio::select! { biased; … }`, the shape `arm_delegate_silence_watch` already uses. A pane already mid-close gets a pre-resolved receiver, closing the register-after-`begin_pane_close` race; abandoned senders are pruned on the next call so the waiter list tracks live waits. On the normal path the gate is still a real awaited delay, so a `…_BUFFER_MS=1` wait is still a wait (`orchestration/delegate/011`); only the closed-target path short-circuits, and it abandons rather than writing early, consistent with `begin_pane_close`'s stance that a close in flight abandons everything touching the pane even if the close then fails.

Neither fix touches the TUI↔daemon contract: both live entirely inside `AgentPtyRegistry`'s in-process delegation tracker, `WorkDoneSignal` is unchanged, and no field changed meaning — so no `PROTOCOL_VERSION` bump and no `.breaking.md` fragment (rule 12).
...
### M6 close-out — as posted

Done after merge. PR #271 merged 2026-07-30T18:04:32Z as `67302b9` (merge commit, not squashed — history preserved); issue #249 closed; all eleven branch commits and all six `orchestration/delegate/010-015` catalog entries are on `main`. At close-out time the fix is on `main` but **not in a tagged release** — the latest tag is `v0.35.1`, which does not contain the merge — so both comments say it lands in the first release after that rather than implying it is already installable.

- **[#199 closed](https://github.com/vfarcic/dot-agent-deck/issues/199#issuecomment-5135748633)** with one comment describing what actually landed (the 1000 ms readiness buffer and its env override, the identity-guarded pointer write, the silent-worker diagnostic) and, deliberately at equal length, what it does **not** establish: the buffer is a stopgap that waits an interval rather than detecting readiness (#243 is the durable answer); the 1000 ms default is the spawn path's warm-case 500 ms doubled for a cold start, with headroom over `orchestration/delegate/012`'s **656 ms fixture** boundary — stated explicitly as the harness's number and not a real-agent measurement, including that an earlier PRD draft got this wrong; and OpenCode exposure is **unproven, not fixed**, since three zero-buffer attempts all passed. All five reporters were asked to confirm and to reopen if the prompt is still dropped, with the recurrence path spelled out: the new orchestrator-pane notice, `RUST_LOG=pane_write=trace`, and raising `DOT_AGENT_DECK_DELEGATE_READINESS_BUFFER_MS` — the last of which would itself be the first real evidence about the readiness window #243 needs.
- **[#243 annotated and left open](https://github.com/vfarcic/dot-agent-deck/issues/243#issuecomment-5135757556)** (no duplicate issue filed): `DELEGATE_READINESS_BUFFER` / `DOT_AGENT_DECK_DELEGATE_READINESS_BUFFER_MS` is recorded as a **second** load-bearing timing constant for that issue to retire alongside the 30 s `SESSION_START_WAIT_TIMEOUT`. The argument for retirement rather than tuning: it is a fixed sleep standing in for a readiness signal that does not exist, sitting immediately after the very wait #243 is about, so a genuine wrapper-side "TUI ready" signal makes it *deletable*. The comment also flags what the replacement must preserve — the buffer's env override is currently the only operator escape hatch for #199's failure class, and M3's notice is the diagnosable surface behind it.
- **Not done by the agent**: `gh issue close 199` was denied by Claude Code's auto-mode permission classifier and was left for a human to run rather than worked around — the same denial that hit this PRD's force-push to PR #271. The comment is public; only the state transition was outstanding at that point.

Everything else on the earlier "still owed" list was already done before merge: M2's buffer-`0` OpenCode observation (settled: not reproducible), the full `cargo test-e2e` gate with `DOT_AGENT_DECK_RECORD=1` (2,542 passed / 0 failed), and the rule 12 previous-release-daemon manual run (pass).
